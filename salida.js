...
operaciones.innerHTML = operacionesData.map(function (operacion) {
	return `<div class="row"><div class="col">${operacion.nombre}</div><div class="col">${operacion.categoria}</div><div class="col">${operacion.fecha}</div><div class="col">${operacion.monto}</div><div class="col-md-4"><button type="submit" class="btn btn-primary">Editar</button><button type="submit" class="btn btn-primary">Eliminar</button></div></div>`;
});
